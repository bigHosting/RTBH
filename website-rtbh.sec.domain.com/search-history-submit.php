...
if (isset($_GET['ip']))
{
        ...
        {
                $rows = $db->rawSelect("SELECT id, inet6_ntop(sourceip) as ip, cidr, type, inserttime, expiretime, hits, country, comment, insertby, allow_edit FROM history WHERE inet6_ntop(sourceip)='$ip'");
        } else {
                $error  = "YES";
                $rows['status'] = " warning ";
...
{
        /* 0 records */
        echo "<tr><td class=\"table_row1\" colspan=\"9\" width=\"100%\"><center><b> STATUS: ". $rows['status']." , Message: ".$rows['message']."<b></b></b></center></td></tr></tbody></table></fieldset><br>";
