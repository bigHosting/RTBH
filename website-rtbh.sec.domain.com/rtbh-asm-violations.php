...
$SEARCH = 'AND Violations:"Mandatory HTTP header is missing"';
$output = "# TOP IP SCRIPT VARIABLES: \n";

if ( isset($_GET['hits']) && is_numeric($_GET['hits']) && ($_GET['hits'] > 300) && ($_GET['hits'] < $MIN_HITS) ){
        $MIN_HITS = $_GET['hits'];
...
function getGraylogMessages($range,$SEARCH){
        ...
        $graylog_api_pass = "GRAYLOG_API_PASSWORD_HERE";

        $query = 'hService:F5_ASM '.$SEARCH;
