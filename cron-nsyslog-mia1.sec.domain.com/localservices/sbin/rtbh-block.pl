...
#
# Changelog:
#     2015.10.08 - added ipv6 compress & expand functions from Net::IP
#     2015.09.30 - added Geopt::Long support
...
use Getopt::Long;                               # script arguments
...
sub display_help {
         print "
usage: $0  -d|--description=mail  -i|--hits=5001  -t|--time=3600  -p|--page=rtbh-mail.php  -v|--verbose=1  [-f|--filename=index.html]  [-m|--method=GET]  [-h|--help]  [-r|--revision]

       $0  -d graylog-mail            -i 5001 -t 3600 -p rtbh-mail.php  -v 0
       $0  -d graylog-indexabuserhtml -i 5001 -t 3600 -p rtbh-web.php   -v 1  -f index.html  -m GET\n\n";
...
GetOptions( \ my %options,
        ...
        'h|help'           => \&display_help,
        'r|revision'       => sub{ print "This is $0, version " .VERSION. "\n"; exit; },
        'v|verbose=i'      => \ my $debug
) or &display_help;
&display_help if ( (scalar(@ARGV < 0)) || (!defined($hits)) || (!defined($debug)) || (!defined($db_string)) || (!defined($time)) || (!defined($page)) );
...
if ( (defined $filename && $filename ne '') && (defined $method && $method ne '') ) {
        $global{'GET_URL'} = "https://rtbh.sec.domain.com/$page?hits=$hits&time=$time&filename=$filename&method=$method";
} else {

        $global{'GET_URL'} = "https://rtbh.sec.domain.com/$page?hits=$hits&time=$time";
}
...
foreach my $ip (keys %records)
{
        ...
        if ($found == '0')
        {
                ...
                &mysql_insert($dbh,'blacklist',$type,$ip,$no_hits,$block_seconds,$n,$geoip_country,$insertby,$db_string);
