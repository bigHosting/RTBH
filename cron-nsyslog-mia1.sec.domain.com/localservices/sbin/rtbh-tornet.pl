...
my %global = (
        MAX_ENTRIES      => 20000,
        GET_URL         => "https://www.dan.me.uk/torlist/",
);

# set to 0 if you don't want debug output
my ($debug)   = 0;

my $db_string = "tornet";              # to differenciate between different types of blocks; used by hService and mysql (comments & insertby)
...
foreach my $line (split( /\n/, $list )) {
        ...
        if ( &valid_ipv4_host ( $line ) )
        {

                $records{$line} = 1;                                          # build hash of IPs,  ipv4
                #print "$line $hits\n";
        }

        if ( &valid_ipv6_host ( $line ) )
        {

                $records{$line} = 1;                                          # build hash of IPs,  ipv6
                #print "$line $hits\n";
        }
}
